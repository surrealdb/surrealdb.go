...
// ExampleSelect_parameterRules demonstrates SurrealDB's parameter rules in FROM clauses
func ExampleSelect_parameterRules() {
	// Rule 1: Parameters ARE allowed at the START of FROM expressions
	user := models.NewRecordID("users", "john")
	sql1, vars1 := surrealql.Select(
		surrealql.Expr(
			"?->knows->users", user,
		),
	).Build()
